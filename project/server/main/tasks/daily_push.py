...
def daily_push():
    ...
    html = f'<html><body><b>Global</b><br/>BTC:&nbsp;{int(portfolio["btc_usd"])}$<br/>ETH:&nbsp;{int(portfolio["eth_usd"])}$<br/>CBBI:&nbsp;{int(cbbi)}%<br/>FNG:&nbsp;{fng[2]}%&nbsp;/&nbsp;{fng[3]}<br/><br/><b>Portfolio</b><br/>Total:&nbsp;{int(portfolio["current"])}$&nbsp;/&nbsp;{portfolio["current_24h"]}%<br/>Binance:&nbsp;{int(portfolio["binance_total"])}$&nbsp;/&nbsp;{portfolio["binance_total_24h"]}%<br/>Bitmex:&nbsp;{int(portfolio["bitmex_total"])}$&nbsp;/&nbsp;{portfolio["bitmex_total_24h"]}%<br/><br/><b>Jobs</b><br/>Successful:&nbsp;{successful}<br/>Failures:&nbsp;{failures}<br/>Lifetime:&nbsp;{worker_stats["lifetime"]}<br/>Working time:&nbsp;{worker_stats["working_time"]}<br/></body></html>'
